...
void main() {
    ...
    float d = distance(vUv, center);
    float g = exp(-d * d / (radius * radius * 0.5));
    c.rgb = min(c.rgb + absorbance * (pigAmt * g), vec3(${MAX_ABSORBANCE.toFixed(1)} * 2.0));
    gl_FragColor = c;
}
`;
...
void main() {
    ...
    vec3 A = texture2D(depTex, vUv).rgb + texture2D(pigTex, vUv).rgb;
    vec3 T = exp(-A);
...
void main() {
    ...
    vec3 A = texture2D(depTex, uv).rgb + texture2D(pigTex, uv).rgb;
    vec3 T = exp(-A);
